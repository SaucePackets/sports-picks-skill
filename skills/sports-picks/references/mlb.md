...
- Conservative edge = `conservative_probability - polymarket_ask`. Polymarket US charges ZERO trading fees (confirmed 0 bps on every executed receipt) — do NOT subtract a phantom fee. Cardable requires conservative edge >= the shared policy `min_conservative_edge` (currently 0.05), loaded from `risk_limits.json` `mlb_policy` via `scripts/mlb_runtime_policy.py`; never hard-code a threshold.
- `conservative_probability` = your raw handicapped probability minus the documented `uncertainty_haircut` (small samples, opener/bulk uncertainty, contact/HR dependence, unavailable leverage relievers, unconfirmed lineups). The haircut is model uncertainty, NEVER a venue fee.
...
- Record `dk_fair_prob`, `raw_probability`, `uncertainty_haircut`, `conservative_probability`, `current_ask`, `projected_edge_at_current_ask`, and `model_version` on every schedule candidate — a candidate missing any of these is ineligible for standing-authorized routing, and the edge is recomputed live at review and at execution; the stored morning edge is never the executed edge.
- Volume rails (same shared policy): at most `max_mlb_official_bets_per_day` official MLB bets per day (currently 2, ranked by conservative edge then baseball-gate strength) and at most one Small-tier bet per day during probation. Starter-pending watchlist promotions are disabled while `starter_pending_promotions_enabled` is false.
