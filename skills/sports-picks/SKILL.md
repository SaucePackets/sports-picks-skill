...
- Jerry may ask for a deeper pass afterward. When he asks, start from the latest same-day cron proposed card / reflection handoff if available; deepen that candidate instead of rerunning the whole slate from scratch. Re-verify current price, injuries/lineups, weather, and game status before final judgment. If the cron output is missing, stale, or the user asks for a different game, run fresh analysis.
