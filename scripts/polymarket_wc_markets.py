...
def load_world_cup_data() -> dict[str, Any]:
    ...
    try:
        return fetch_data_route(build_id)
...
def extract_markets(data: dict[str, Any], date: str | None = None) -> list[MatchMarket]:
    markets: dict[str, MatchMarket] = {}
    for obj in walk_dicts(data):
        slug = obj.get("slug")
        title = obj.get("title") or obj.get("question")
        ...
        raw_markets = obj.get("markets")
        market_rows: list[Any] = raw_markets if isinstance(raw_markets, list) else [obj]
        ...
        for row in market_rows:
            if not isinstance(row, dict):
                continue
            candidate_prices = row.get("outcomePrices")
            outcomes = row.get("outcomes")
            if isinstance(candidate_prices, list) and len(candidate_prices) >= 2:
                prices = candidate_prices
                volume = volume if volume is not None else parse_price(row.get("volume") or row.get("volumeNum"))
                liquidity = liquidity if liquidity is not None else parse_price(row.get("liquidity") or row.get("liquidityNum"))
        ...
        yes_team = teams[0] if teams else None
        no_team = teams[1] if len(teams) > 1 else None
        markets[slug] = MatchMarket(
            slug=slug,
            title=title,
            teams=teams,
            yes_team=yes_team,
    ...
    return sorted(markets.values(), key=lambda market: market.slug)
...
def main(argv: list[str] | None = None) -> int:
    ...
    markets = extract_markets(data, date=args.date)
